...
- **`okf-ingest html`** — a navigable static site (one page per concept, links rewritten
...
- **`okf-ingest graph`** — one interactive force-directed page (hand-rolled canvas JS, no
...
- **`okf-ingest export --mermaid`** — a Mermaid `graph LR` diagram for embedding in
  markdown; `okf-ingest export` alone emits portable `{nodes, edges}` JSON.
