...
Every concept row carries a `content_hash`. `okf-ingest ingest --incremental` diffs each
...
`okf-ingest embed --incremental` re-embeds only concepts whose content changed — the
